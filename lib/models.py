...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_chaincoind(self, chaincoind, rec):
        ...
        object_hex = chaincoinlib.SHIM_deserialise_from_chaincoind(object_hex)
        objects = chaincoinlib.deserialise(object_hex)
        subobj = None
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/ChaincoinDrive, etc.)
        return False
    ...
    def prepare(self, chaincoind):
        try:
            object_hash = chaincoind.rpc_command(*self.get_prepare_command())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = CHAINCOIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, chaincoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - chaincoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, chaincoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - chaincoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, chaincoind):
        now = int(time.time())
        return (self.created_at < (now - chaincoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, chaincoind):
        if self.is_expired(chaincoind):
            return False

        return True

    def is_deletable(self, chaincoind):
        if self.is_expired(chaincoind):
            return True
